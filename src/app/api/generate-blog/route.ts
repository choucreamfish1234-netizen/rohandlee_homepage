...
2. 마크다운 형식으로 작성합니다.
3. 제목(h1)은 포함하지 않습니다(별도로 제공됩니다).
4. h2, h3 소제목을 적절히 활용하여 구조화합니다.
5. 실제 판례나 법률 조항을 인용할 때는 정확한 정보만 포함합니다.
6. 글 마지막에는 "전문 변호사와 상담하세요"라는 취지의 자연스러운 마무리를 포함합니다.
7. SEO에 최적화된 제목, 요약, 태그를 함께 생성합니다.
8. 글의 분량은 1500~3000자 내외로 합니다.

[네이버 블로그용 규칙]
- HTML 형식으로 작성 (네이버 에디터에 HTML로 붙여넣기용)
- 소제목은 <h3> 태그, 굵은 글씨는 <b> 태그
...
반드시 아래 JSON 형식으로 응답하세요:
{
  ...
  "naverContent": "네이버 블로그용 본문 (HTML, 네이버 스타일)",
  "excerpt": "2줄 내외의 짧은 요약",
  "summary": ["핵심 1", "핵심 2", "핵심 3"],
  "tags": ["태그1", "태그2", "태그3"],
  "meta_description": "SEO 메타 설명 (160자 이내)",
  "estimatedReadTime": "N분"
}`

    const userPrompt = `주제: ${topic}\n카테고리: ${category || '일반'}\n\n위 주제에 대한 법률 블로그 글을 홈페이지용(마크다운)과 네이버용(HTML) 두 가지 버전으로 작성해주세요.`
