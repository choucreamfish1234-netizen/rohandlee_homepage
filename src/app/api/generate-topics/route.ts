...
export async function POST(req: NextRequest) {
  try {
    ...
    if (categories.length === 0) {
      categories = Object.keys(CATEGORY_AUTHORS)
    }

    const totalTopics = 30
    const perCategory = Math.floor(totalTopics / categories.length)
    const remainder = totalTopics % categories.length

    const distribution = categories.map((cat, i) => {
      const count = perCategory + (i < remainder ? 1 : 0)
      const author = CATEGORY_AUTHORS[cat] || '이유림 변호사'
      return `- ${cat} ${count}개 (작성자: ${author})`
    }).join('\n')
...
JSON만 응답. 다른 텍스트 금지.`,
        messages: [{
          role: 'user',
          content: `기존 블로그 제목 목록 (중복 금지):\n${existingTitles.join('\n')}\n\n위 제목과 겹치지 않는 새로운 주제 ${totalTopics}개를 JSON 배열로 생성해주세요.`,
        }],
      }),
    })
