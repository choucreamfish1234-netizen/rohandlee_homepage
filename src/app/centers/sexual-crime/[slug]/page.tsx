...
import { SEXUAL_CRIME_TYPES, findCrimeType } from '@/data/sexual-crime-types'
import CrimeTypeDetail from './CrimeTypeDetail'
...
export function generateStaticParams() {
  return SEXUAL_CRIME_TYPES.map(t => ({ slug: t.slug }))
}

export async function generateMetadata({ params }: Props): Promise<Metadata> {
  const { slug } = await params
  const data = findCrimeType(slug)
  if (!data) return { title: '성범죄 피해자 전담센터' }

  const pageUrl = `${baseUrl}/centers/sexual-crime/${slug}`
  return {
